...
var HighChart = {
    ChartDataFormate: {
        ...
        FormatGroupData2: function (data) {//处理分组数据，数据格式：name：XXX，group：XXX，value：XXX用于折线图、柱形图（分组，堆积）
            var names = new Set();
            var series = new Array();
            var group = new Set();
            for (var i = 0; i < data.length; i++) {
                names.add(data[i].dname);
                group.add(data[i].dgroup);
            }
            for (var i = 0; i < group.size(); i++) {
                ...
                for (var j = 0; j < data.length; j++) {
                    for (var k = 0; k < names.size(); k++)
                        if (group.get(i) == data[j].dgroup && data[j].dname == names.get(k))
                            temp_data.push(data[j].dvalue);
                }
                temp_series = { name: group.get(i), data: temp_data };
                series.push(temp_series);
            }
            return { category: names.elements, series: series };
        },
    ...
    ChartOptionTemplates: {
        ...
        Line: function (data, name, title) {
            var line_datas = HighChart.ChartDataFormate.FormatGroupData2(data);
            var option = {
                title: {
                    text: title || '',
                    x: -20
        ...
        SPLine: function (data, yName, title) {
            var line_datas = HighChart.ChartDataFormate.FormatGroupData2(data);
            var option = {
	    		chart: {
	    	       type: 'spline'
...
//                        }
        ...
        RoomLine: function (data, xName, yName, contentName, title) {
            var option = {
        		chart: {
                    zoomType: 'x'
                },
