...
(function (window, $, undefined) {
    ...
    //  判断当前 String 对象是否是正确的电话号码格式。
    coreString.isTel = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        return /^(\d{1,4}\-)?((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(str);
    };
    ...
    coreString.isMobile = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        return /^(\d{1,4}\-)?(13|14|15|17|18)\d{9}$/i.test(str);
    };
    ...
    //将字符串对象转换成字符串数组
    coreString.toArray = function (str,splitStr) {
        var strArray = [];
        if(coreString.isNullOrEmpty(str)){
            return strArray;
        }
        str = coreString.trim(str);
        splitStr = coreString.isNullOrEmpty(splitStr) ? "," : String(splitStr);
        var temp = str.split(',');
        for (var i = 0; i < temp.length; i++) {
            strArray.push('' + temp[i]);/* 避免转换成数字 */
        }
        return strArray;
    };
    coreString.prototype.toArray = function (splitStr) { return coreString.toArray(this,splitStr); };
    ...
    coreDate.isDate = function (date) { return coreUtil.isDate(date) || coreString.isDate(date) || new Date(date) != 'NaN'; };
    ...
    coreDate.compare = function (date1, date2) {
        date1 = coreDate.isDate(date1) ? date1 : false;
        date2 = coreDate.isDate(date2) ? date2 : false;
        if(!date1 || !date2){
            return false;
        }
        var d1 = $.string.isString(date1) ? $.string.toDate(date1) : date1.getTime();
        var d2 = $.string.isString(date2) ? $.string.toDate(date2) : date2.getTime();
    ...
    coreDate.addMonths = function (date, months) {
        if (!coreUtil.isDate(date)) { date = new Date(Date.parse(date.replace(/-/g,   "/"))); }
        if (!coreUtil.isNumeric(months)) { months = 0; }
        return new Date(date.getFullYear(), date.getMonth() + Number(months), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds());
    };
    ...
    coreDate.addYears = function (date, years) {
        ...
        return new Date(date.getFullYear() + Number(years), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds());
    };
    ...
    coreDate.format = function (date, format) {
        if (!coreDate.isDate(date)) { return null; };
        format = coreString.isNullOrWhiteSpace(format) ? "yyyy-MM-dd" : format;
        switch (typeof date) {
            case "string":
    ...
    coreJquery.prototype.serializeObject = function (options) {
        ...
        //20131109 增加默认处理
        if(!options) {
            options = {onlyEnabled: false, transcript: "overlay", overtype: "append", separator: ","};
        }
        options = options || {};
        ...
        function getElemVal(elem) {
            ...
            if(rsubmittable_radiocheckbox.test(elem.type) && !elem.checked){
                return "";
            }
            return elem.val;
        };
    ...
    coreJquery.shine = function (selector, interval, times) {
        ...
        var a = function () { selector.addClass("vt-shine"); };
        var b = function () { selector.removeClass("vt-shine"); };
        var run = function () {
    ...
    coreUtil.addCss(".vt-shine { filter: alpha(opacity=40); opacity: 0.4; }");
